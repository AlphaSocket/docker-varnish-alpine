...
ENV \
	...
	CONFIG_VARNISH_STARTUP_ADDITIONAL_OPTIONS='-T 127.0.0.1:${BUILD_VARNISH_CONTROL_PANEL_PORT} -p cli_buffer=16384 -p feature=+esi_ignore_other_elements -p vcc_allow_inline_c=on'
...
ADD imports/bin/setup /usr/local/bin/setup/1518701524
ADD imports/bin/config /usr/local/bin/config/1518701524
...
RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1518701524 1>/dev/stdout 2>/dev/stderr

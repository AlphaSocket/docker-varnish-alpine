...
ENV \
	...
	BUILD_CMD="echo $CONFIG_VARNISH_STARTUP_OPTIONS $CONFIG_VARNISH_STARTUP_OPTIONS_ADDITIONAL | envsubst | xargs -exec varnishd " \
...
ADD imports/bin/setup /usr/local/bin/setup/1518660028
ADD imports/bin/config /usr/local/bin/config/1518660028
...
RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1518660028 1>/dev/stdout 2>/dev/stderr

...
ENV \
	...
	BUILD_CMD="eval "varnishd $CONFIG_VARNISH_STARTUP_OPTIONS $CONFIG_VARNISH_STARTUP_OPTIONS_ADDITIONAL"" \
...
ADD imports/bin/setup /usr/local/bin/setup/1518576209
ADD imports/bin/config /usr/local/bin/config/1518576209
...
RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1518576209 1>/dev/stdout 2>/dev/stderr

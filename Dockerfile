...
ENV \
	...
	BUILD_PORTS_MAIN="80" \
	BUILD_PORTS_ADDITIONAL=" 6082 6085" \
	...
	SETUP_DEPENDENCIES_BUILD="unzip make" \
...
ADD imports/bin/setup /usr/local/bin/setup/1521468051
ADD imports/bin/config /usr/local/bin/config/1521468051
...
RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1521468051 1>/dev/stdout 2>/dev/stderr

...
ENV \
	...
	CONFIG_VARNISH_STARTUP_ADDITIONAL="-T 127.0.0.1:6082 -p cli_buffer=16384 -p feature=+esi_ignore_other_elements -p vcc_allow_inline_c=on"
...
ADD imports/bin/setup /usr/local/bin/setup/1518575013
ADD imports/bin/config /usr/local/bin/config/1518575013
...
RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1518575013 1>/dev/stdout 2>/dev/stderr
